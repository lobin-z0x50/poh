...
# Stringクラスを拡張
class String
  # split して to_i する。テストデータ用に '#' 以降はコメントとして無視する。
  def split_to_i
...
class CardSet
  ...
  # 文字列からインスタンスを作る
  # @param [String] str スペース区切りのカード値を表す文字列
  # @return [CardSet]
  def self.parse(str)
  ...
  # 手札の評価値を計算する
  # @return [Array<Integer>] 21以下の評価値を配列で返す
  # 例：
  #   3 5 9  -> [17]
  #   6 7 9  -> []
  #   1      -> [1, 11]
...
class Player
  ...
  # BET枚数を決める
  # @param [RoundContext] round_ctx
  # @return BET枚数。小数を返す可能性もある。
  def bet(round_ctx)
    [round_ctx.tips, BET_LIMIT].min
  end

  # 手を考える
  # @param [GameContext] ctx
  # @return [Symbol] :HIT or :STAND
  def think(ctx)
...
# コンテキストを作ってプレイヤークラスに処理させる
str = gets.split_to_i
if str[0] == 0
  rctx = RoundContext.new(str[1], gets.to_i, gets.to_i)
  puts '%d'%Player.new.bet(rctx)   # 整数化しておく
else
