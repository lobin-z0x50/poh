...
def bi_search(area)
  ans = Array.new($H){ Array.new($W) } # 答えを詰めていくための２次元配列
  ...
  area.each do |row|
    ...
    __yoko_search(current, row).each do |sizes|
      h = sizes[0] - 1
      (0...sizes[1]).each do |w|
  ...
  Enumerator.new do |y|
    ...
    (0...row.length).each do |i|
      ...
      if row.getbyte(i) == 0x30
        (0...current[i]).each { |j| sum[j] = (sum[j]||0) + 1 }
        sum[current[i]] = 0
      else
        sum.map!{0}
      end
